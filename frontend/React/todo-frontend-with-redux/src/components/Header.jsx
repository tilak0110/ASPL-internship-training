...
const Header = () => {
  ...
  const { theme, toggleTheme } = useTheme(); // still using ThemeContext

  const { user } = useSelector((state) => state.auth); // 🔁 Redux instead of useAuth
  const [showModal, setShowModal] = useState(false);

  const handleLogout = () => {
    dispatch(authActions.logout()); // 🔁 Redux logout
    setShowModal(false);
    navigate("/");
  };
